...
namespace GoFish_WPF
{
    class Player
    {
        ...
        public Player(String name, Random random, Game gameInstance)
        {
            this.name = name;
            this.random = random;
            this.gameInstance = gameInstance;
            gameInstance.AddProgress(Name + " has just joined the game");
        ...
        public Deck DoYouHaveAny(Value value)
        {
            ...
            if (pulledValues.Count != 0)
                gameInstance.AddProgress(Name + " has " + pulledValues.Count + " " + Card.Plural(value));
            else
                gameInstance.AddProgress(Name + " has 0 " + Card.Plural(value));
            return pulledValues;
        }
        ...
        {
            gameInstance.AddProgress(Name + " asks if anyone has a " + value + ".");
            ...
            if(numberOfCardsAdded == 0)
            {
                ...
                {
                    cards.Add(stock.Deal());
                    gameInstance.AddProgress(Name + " had to draw from the stock.");
                }
